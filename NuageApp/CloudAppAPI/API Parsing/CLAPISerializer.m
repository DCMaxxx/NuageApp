...
/*----------------------------------------------------------------------------*/
+ (NSData *)settingForGlobalPrivacy:(BOOL)privacy {
    return [self JSONDataFromDictionary:    @{
                                                @"user":    @{
                                                    @"private_items": @(privacy)
                                                }
                                            }];
}

+ (NSData *)changeToEmail:(NSString *)email withPassword:(NSString *)password {
    return [self JSONDataFromDictionary:    @{
                                                @"user":    @{
                                                    @"email": email,
...
+ (NSData *)changeToPassword:(NSString *)newPassword withCurrentPassword:(NSString *)password {
    return [self JSONDataFromDictionary:    @{
                                                @"user":    @{
...
+ (NSData *)updateCustomDomain:(NSString *)customDomain customDomainHome:(NSString *)customDomainHome {
    NSDictionary * dic;
    if (!customDomainHome) {
        dic =   @{
                    @"user":    @{
        ...
        dic =   @{
                  @"user":    @{
                          @"domain":customDomain,
                          @"domain_home_page":customDomainHome
                          }
                  };
        
    }
    return [self JSONDataFromDictionary:dic];
}
